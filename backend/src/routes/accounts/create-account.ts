...
export const CreateAccount = async (app: FastifyInstance) => {
  app.post("/createAccount", async (request: FastifyRequest, reply) => {
    try {
      ...
      const [accountId] = await knex<Account>("Conta")
        .insert({
          titulo: accountData.titulo,
          foto_referencia: accountData.foto_referencia,
      ...
      for (const item of accountData.dados) {
        await knex<ItemConta>("ItemConta").insert({
          rotulo: item.label,
          dado: item.value,
          id_conta: accountId.id, // relaciona cada item a conta criada
        });
      }
      ...
      if (error instanceof ZodError) {
        ...
        return reply
          .status(400)
          .send({ error: error.errors || "Erro ao criar conta" });
      } else {
        console.error("Erro ao criar a conta:", error);
